                ...
                <div class="mb-[20px] rounded-xl bg-white p-[20.32px] shadow-[0_3.3px_12px_rgba(0,0,0,0.15)]">
                            ...
                            <div>
                                <div class="mb-[6.77px] flex flex-col gap-[3.39px]">
                                    ...
                                    @if ($item->shipping_type == 'on_location')
                                        <input type="text" value="On Location" readonly
                                            class="rounded-[10px] border border-[#969EBA] px-[13.54px] py-[6.77px]"
                                            placeholder="Masukan Kurir">
                                    @endif
                                    @if ($item->shipping_type == 'on_send')
                                        <input type="text" value="JNE Reguler" readonly
                                            class="rounded-[10px] border border-[#969EBA] px-[13.54px] py-[6.77px]"
                                            placeholder="Masukan Kurir">
                                    @endif
                                </div>
                                ...
                                @if ($item->shipping_type == 'on_send')
                                    <div class="flex flex-col gap-[3.39px]">
                                        ...
                                        <input type="text" id="shipping_code_input_{{ $item->id }}"
                                            class="rounded-[10px] border border-[#969EBA] px-[13.54px] py-[6.77px]"
                                            placeholder="Masukan No Resi" value="{{ $item->shipping_code }}">
                                    </div>
                                @endif
                    ...
                    <div class="flex justify-end">
                        @if ($item->shipping_status == 'packing')
                            <button onclick="all_order_{{ $item->id }}.showModal()"
                                class="rounded-[10px] bg-primary px-[20.32px] py-[6.77px] text-[17px] font-semibold text-white">
                                ...
                                <div class="modal-box">
                                    <form method="dialog">
                                        <button class="btn btn-circle btn-ghost btn-sm absolute right-2 top-2"
                                            id="button-close-all-{{ $item->id }}">✕</button>
                                    </form>
                                    <div>
                                        ...
                                        <form action="{{ url('/admin/order') }}" method="post">
                                            ...
                                            @csrf
                                            @if ($item->shipping_type == 'on_send')
                                                <input type="text" class="hidden" name="shipping"
                                                    id="shipping_code_value_{{ $item->id }}">
                                            @endif
                                            @if ($item->shipping_type == 'on_location')
                                                <input type="text" class="hidden" name="shipping" value="on_location">
                                            @endif
                                            <input type="text" name="user_payment_id" value="{{ $item->id }}"
                                                class="hidden" readonly>
                                            <div class="flex justify-center gap-5">
                                                <button class="btn btn-primary btn-outline w-32" type="button"
                                                    onclick="document.getElementById('button-close-all-{{ $item->id }}').click()">Tidak</button>
