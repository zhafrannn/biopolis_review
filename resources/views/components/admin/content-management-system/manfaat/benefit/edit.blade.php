<dialog id="edit_benefit{{ $benefit->id }}" class="modal overflow-hidden">
    <div class="modal-box w-11/12 max-w-5xl">
        <div class="px-[20px] overflow-hidden">
            ...
            <form action="{{ url('/admin/content-management-system/benefit/update/'. $benefit->id ) }} " method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="flex">
                    <div class="mt-9 flex w-full flex-col">
                        ...
                        <input type="text" class="h-[56px] rounded-xl border border-[#E5E5E5] px-[24px] text-[#0A0A0B]" name="title" value="{{$benefit->title}}">
                    </div>
                </div>
                <div class="mt-9 flex flex-col">
                    <label for="" class="text-[13.54px] text-[#0A0A0B]">Keterangan</label>
                    <textarea class="flex h-[98px] items-center justify-center rounded-xl border border-[#E5E5E5] px-[20px] pt-2 text-[#0A0A0B]" name="description">{{ $benefit->description}}
                    </textarea>
                </div>
            </form>
        </div>
